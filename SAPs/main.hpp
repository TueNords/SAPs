...
void ProduceStep(std::vector< std::vector< int > > &lookuptable, std::vector<std::vector<int>> &grid, mpz_class &counter, const int &length,unsigned int row,unsigned int col, int remaining, int depth);    //Produce queue jobs
void TakeStep(std::vector< std::vector< int > > &lookuptable, std::vector<std::vector<int>> &grid, mpz_class &counter, unsigned int row, unsigned int col, int remaining);       //General walking function
...
void WorkerFunc(std::vector< std::vector< int > > &lookuptable, int remaining, unsigned int n, unsigned int d, bool isHost);                      //Retrieves jobs from queue en proceeds calculating them with TakeStep if arguments given it will behave as the reporting host thread
